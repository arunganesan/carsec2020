...
\paragraph{Challenges in Estimating Vehicular Sensor Values} \watchfone{} uses the
smartphone's multifaceted sensors to cross-validate vehicular sensors reported on the
CAN bus. Specifically, it uses 3 widely-found smartphone sensors (Inertial Measurement
Unit (IMU), magnetometer, and GPS) to estimate 6 vehicular sensors (gear, engine RPM,
speed, steering wheel angle, odometer, and consumed fuel).

This is a challenging task due to the lack of a direct mapping between smartphone and 
vehicular sensor values. For example, the steering wheel angle has some relation to the 
angular motion of the vehicle, but this is not directly observable by the smartphone. 
To address this, we used vehicle's physical dynamics models (e.g., Ackerman model 
\cite{liu_bigroad:_2017}), publicly available vehicle specs from the OEMs (e.g., gear 
ratios), and sensor fusion techniques (e.g., complementary filter) to 
translate the smartphone's measurements to the vehicle sensors.
...
\item \changes{{\em Expansion of vehicle estimation research}. We extend prior work on vehicle estimation algorithms by (a) a novel neural-network based gear-estimation algorithm and (b) an investigation into the failure modes of engine RPM estimation using prior methods}
