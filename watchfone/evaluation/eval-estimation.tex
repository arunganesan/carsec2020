...
Fig.~\ref{fig:estimation-accuracy} plots the CDF of the errors.
...
1 gear position. 
We noticed that the errors are related to where the 
...
\begin{figure}
    ...
    \caption{\color{highlightColor} Vehicle speed estimation error for increasing high-frequency GPS noise.
...
movement. {\color{highlightColor}We simply do this using the gravity vector.} In contrast,
...
Our key findings in estimating 6 vehicular sensors using 3 phone sensors can be summarized as follows. {\color{highlightColor}The first three key findings reflect and corroborate similar findings in related work. The fourth key finding is an extension based on our experiments with passenger phones}.
\begin{itemize}
    ...
    \item Error of steering wheel angle estimation and gear estimation are negatively correlated with vehicle speed --- the faster the car, the lower the estimation error.
